                      ...
                      uint64_t *ecid, char *serial, size_t serial_len)
{
    ...
    /* Read the serial string descriptor.
        ...
        if (dev) {
            ...
            if (libusb_get_device_descriptor(dev, &desc) == LIBUSB_SUCCESS) {
                if (desc.iSerialNumber > 0) {
                    ...
                    log_warn("Device reports iSerialNumber index is 0, using fallback index %d", DFU_SERIAL_INDEX);
                }
            } else {
                log_warn("Failed to retrieve device descriptor, using fallback index %d", DFU_SERIAL_INDEX);
            }
        }
